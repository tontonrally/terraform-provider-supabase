...
func (d *APIKeysDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	httpResp, err := d.client.V1GetProjectApiKeysWithResponse(ctx, data.ProjectRef.ValueString(), &api.V1GetProjectApiKeysParams{Reveal: &reveal})
	if err != nil {
	...
	for _, key := range *httpResp.JSON200 {
		if key.Type.IsSpecified() && !key.Type.IsNull() {
			...
			switch keyType {
			case api.ApiKeyResponseType("legacy"):
				if key.Name == "anon" {
			...
			case api.ApiKeyResponseType("publishable"):
